	...
	while True:
		...
		if ABNORMAL_CHECK:
			if ABNORMAL:
				...
				for track in humans_detected:
					if track.track_id in abnormal_individual:
						[x, y, w, h] = list(map(int, track.to_tlbr().tolist()))
						cv2.rectangle(frame, (x , y ), (w, h), RGB_COLORS["red"], 5)
				beep_sound.play(-1)  # Loop the sound indefinitely until stopped
			else:
			...
			if ab_warning_timeout > 0:
				...
				# Stop the beep sound when the warning timeout is over
				beep_sound.stop()
